...
export default {
  ...
  watch: {
    ...
    currentMenuIndex(newVal) {
      sessionStorage.setItem('Oms-CurrentMenuIndex', newVal)
      let newValArr = newVal.split(':') // 头部菜单权限标识是否有子应用标识
      let newValFlag = newValArr[0] // 为子应用入口时，标识含有 'childApp'
      let newValPath = newValArr[newValArr.length - 1]
      ...
      let isChildAppPath = this.currentGlobalRedirectRoute.path && CHILD_PROJECT.some(it => this.currentGlobalRedirectRoute.path.includes(`/${it.name}/`))
      // newValFlag === 'childApp' 直接点击头部菜单进入子应用，或者，isChildAppPath == true 跳转路由为子应用路由
      if (newValFlag === 'childApp' || isChildAppPath) {
        ...
        if ((this.oldChildAppPath.includes(`/${childAppName}/`)) && this.childRouters && this.childRouters.length) {
...
export default {
  ...
  watch: {
    ...
    childBreadcrumb: {
      handler(newVal) {
        console.log('childBreadcrumb', newVal)
        if (!window.isInQiankun) return
...
<script>
  export default {
    ...
    watch: {
      ...
      childRouters: {
        handler(newVal) {
          console.log('childRouters', newVal)
          if (!window.isInQiankun) return
          this.menus = newVal
        },
        deep: true,
...
在录入基座应用菜单时，需要设置子应用的入口，方便点击头部菜单进入子应用（可以是跳转到子应用的首页，或者是指定页面）
...
[
  ...
  },
  {
    ...
    "parentId": 0,
    "type": 1,
    "name": "child-01", // 子应用头部菜单名
    "iconUrl": "",
    "component": "",
    "path": null,
    "permissionFlag": "childApp:/child-01/", // 子应用菜单标识（格式为：childApp:子应用入口路由）
    "sort": 5
  },
  {
    "id": 2406,
    ...
    "component": "",
    "path": null,
    "permissionFlag": "childApp:/child-02/", // 子应用菜单标识（格式为：childApp:子应用入口路由）
    "sort": 6
  }
]
```
